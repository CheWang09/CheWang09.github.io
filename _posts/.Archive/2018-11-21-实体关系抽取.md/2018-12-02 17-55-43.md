...
To evaluate this PCNNs method, we select the following two methods for comparison. FP-Growth represents a traditional model that is a method of association rule learning. Another is CNN which is a deep learning method was proposed by LeCun, Yann. Figure 2 shows the precision-recall curves for each method, where PCNNs denotes our method, and demonstrates that PCNNs achieves higher prcision over the entire range of recall. 
