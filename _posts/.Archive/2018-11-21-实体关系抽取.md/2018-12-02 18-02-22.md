...
To evaluate this PCNNs method, we select the following two methods for comparison. One method is FP-Growth which represents a traditional model that is a method of association rule learning. Another is CNN which is a deep learning method was proposed by LeCun, Yann. Figure 2 shows the precision-recall curves for each method, where PCNNs denotes our method, and demonstrates that PCNNs achieves higher prcision over the entire range of recall. In terms of both precision and recall, PCNNs outperforms all other evaluated approaches. Since FP-growth is a method based on assocation rules, it would discover regularities between entities from our sentence. Even if in this particular situation, in recruitment area, it has a good performance, but it did not consider the semantic of sentence context. 
