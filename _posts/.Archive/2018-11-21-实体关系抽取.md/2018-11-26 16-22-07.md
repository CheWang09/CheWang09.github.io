...
In the example shown in Figure 1, we assume that the length of the filter is w (w = 3); thus, $w \in \mathbb{R}^m $(m  =w * d). We 
m (m = w∗d).
We consider S to be a sequence {q1, q2, · · · , qs},
where qi ∈ R
d
. In general, let qi:j refer to the
concatenation of qi
to qj . The convolution operation
involves taking the dot product of w with
